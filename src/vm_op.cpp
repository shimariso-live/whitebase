...
int create_vm(const std::string& vmname, const std::string& volume, uint32_t memory, uint16_t cpu, std::optional<uint32_t> data_partition, bool stub)
{
    auto vm_dir = vm_root / vmname;
    ...
    auto volume_dir = get_volume_dir(volume, [](auto name) -> std::filesystem::path {throw std::runtime_error("Volume " + name + " does not exist or offline");});
    ...
    try {
        ...
        auto vm_ini = volume_vm_dir / "vm.ini";
        {
            std::ofstream f(vm_ini);
            f << "memory=" << std::to_string(memory) << std::endl;
        ...
        if (stub) {
            ...
            auto dst_localtime = stubroot / "etc" / "localtime";
            if (std::filesystem::exists(localtime) && !std::filesystem::exists(dst_localtime)) {
                std::filesystem::copy(localtime, dst_localtime, std::filesystem::copy_options::copy_symlinks);
            }
            ...
            if (_home) {
                ...
                auto dest_authorized_keys = stubroot / "root" / ".ssh" / "authorized_keys";
